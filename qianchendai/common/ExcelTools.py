...
class DoExcel:
    ...
    def read_excel(self):
        ...
        for row in range(2, self.sheet.max_row+1):
            ...
            case.headers = self.sheet.cell(row, 4).value
            case.data = self.sheet.cell(row, 5).value
            case.method = self.sheet.cell(row, 6).value
            case.expected = self.sheet.cell(row, 7).value
            case.check_sql = self.sheet.cell(row, 10).value
            cases.append(case)
        self.lw.close()
        return cases
...
if __name__ == '__main__':
    do_excel = DoExcel(constant.excel_dir,'register')
